...
export const crossTasks = (events, date, startTime, endTime) => {
  ...
  const eventsPerDay = events
    ...
    .map(elem => {
      const from = new Date(elem.start).getHours() * 60 + new Date(elem.start).getMinutes();
      const to = new Date(elem.end).getHours() * 60 + new Date(elem.end).getMinutes();
  ...
  if (eventsPerDay.find(el => el.includes(crossEnd - 1) || el.includes(crossStart + 1))) {
