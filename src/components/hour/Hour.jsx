...
const Hour = ({ dataHour, hourEvents, onDelete, onOpen, openSmallModal }) => {
  return (
    <div className="calendar__time-slot" data-time={dataHour + 1} onClick={openSmallModal}>
      {hourEvents.map(({ id, start, end, title, description }) => {
        ...
        const eventEnd = `${new Date(end).getHours()}:${formatMins(new Date(end).getMinutes())}`;

        return (
          <Event
            key={id}
            height={(new Date(end).getTime() - new Date(start).getTime()) / (1000 * 60)}
